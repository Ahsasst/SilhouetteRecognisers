...
public class Background {
    ...
    protected static int findBackgroundColor(BufferedImage image) {
        ...
        HashMap<Integer, Integer> colorCount = new HashMap<>();// Map to store the color number and its amount in the image

        // Count frequency of each color for boundary pixels
        for (int x = 0; x < width; x++) {
            countColorAmount(colorCount, image.getRGB(x, 0)); // Up border
            countColorAmount(colorCount, image.getRGB(x, height - 1)); // Down border
        }

        for (int y = 0; y < height; y++) {
            countColorAmount(colorCount, image.getRGB(0, y)); // Left border
            countColorAmount(colorCount, image.getRGB(width - 1, y)); // Right border
        }
     ...
     */
    protected static void countColorAmount(HashMap<Integer, Integer> colorCount, int numColor) {
    ...
    protected static int findCommonColor(HashMap<Integer, Integer> colorCount) {
        int maxCount = 0;// The amount of the most common color
